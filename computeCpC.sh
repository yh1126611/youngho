...
# Function to calculate CpC ratio
calculate_cpc() {
    local seq=$(echo $1 | tr '[:lower:]' '[:upper:]')  # Convert to uppercase
    local cpc_count=$(echo $seq | grep -o 'CC' | wc -l)
    local total_count=${#seq}  # Total number of letters in the sequence
    echo "scale=4; ($cpc_count * 2) / $total_count" | bc
}

# Create header for output file
echo -e "Chromosome_Coordinate\tDistance\tCpC_Ratio\tStrand" > $output_file
...
while IFS=$'\t' read -r chrom coord strand; do
    for distance in $(seq -10000 100 10000); do
        ...
        # Calculate CpC ratio
        cpc_ratio=$(calculate_cpc $seq)
        
        # Output results
        echo -e "${chrom}_${coord}\t${distance}\t${cpc_ratio}\t${strand}" >> $output_file
    done
done < $coord_file

echo "CpC ratio analysis complete. Results saved in $output_file"
